...
PUBLIC_SNAPSHOT_URL = os.environ.get(
    "MARKET_PUBLIC_SNAPSHOT_URL",
    "https://josh-temple.github.io/studio-lab-research/assets/data/market/latest.json",
)
REUSE_MAX_AGE_MINUTES = 45.0
SYMBOLS = [
    ("EUR/USD", "EUR/USD", "FX"),
    ("USD/JPY", "USD/JPY", "FX"),
...
def reuse_public_snapshot(now: datetime) -> bool:
    try:
        separator = "&" if "?" in PUBLIC_SNAPSHOT_URL else "?"
        url = f"{PUBLIC_SNAPSHOT_URL}{separator}t={int(time.time())}"
        req = urllib.request.Request(url, headers={"User-Agent": "StudioLab-PublicMarket/0.2"})
        with urllib.request.urlopen(req, timeout=10) as res:
            data = json.loads(res.read().decode("utf-8"))

        generated = parse_timestamp(data.get("generated_at_utc")) if isinstance(data, dict) else None
        if generated is None:
            return False
        age_minutes = (now - generated).total_seconds() / 60.0
        market = data.get("market") if isinstance(data, dict) else None
        health = data.get("health", {}).get("market", {}) if isinstance(data, dict) else {}
        ok_symbols = sum(1 for x in market or [] if isinstance(x, dict) and x.get("status") == "ok")
        if (
            -5.0 <= age_minutes <= REUSE_MAX_AGE_MINUTES
            and health.get("status") == "ok"
            and ok_symbols == len(SYMBOLS)
        ):
            write_snapshot(data)
            print(f"snapshot_reused=true age_minutes={age_minutes:.1f} ok_symbols={ok_symbols}")
            return True
    except Exception as exc:
        print(f"snapshot_reuse_skipped={type(exc).__name__}")
    return False
...
def main() -> int:
    ...
    # Scheduled workflows can be delayed or duplicated. Reuse a healthy recent
    # public snapshot so redundant runs do not spend provider credits.
    if reuse_public_snapshot(now):
        return 0
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={"User-Agent": "StudioLab-PublicMarket/0.2"},
        )

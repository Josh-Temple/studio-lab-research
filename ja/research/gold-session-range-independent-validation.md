...
現時点では、range persistenceを救済するための新しい自動的なholdout追加も優先していません。実務上の判断は、**前セッションのhigh-low rangeを単独の予測メカニズムとして扱う研究の優先度を下げ、未使用データを救済検証に追加投入しない**ことです。

その後のGOLD研究は別の水平線メカニズムへ移り、過去データではpositiveな方向反応が見られた一方、未使用データの無条件タッチ検証ではnegative resultとなりました。これは別仮説の研究であり、今回のsession-range仮説を遡って救済するものではありません。現在の整理は[水平線研究の統合ページ]({{ '/ja/research/gold-horizontal-replication-execution-evidence/' | relative_url }})にまとめています。
